<template>
  <v-card min-width="0">
    <v-toolbar class="secondary" card dark extended>
      ...
      <v-toolbar-title>About Authentication Techniques</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-tooltip top>
        <v-btn slot="activator" icon @click="closeDialog">
          <v-icon>close</v-icon>
        </v-btn>
        <span>Close the dialog</span>
      </v-tooltip>
      <v-tabs color="transparent" slot="extension" fixed-tabs v-model="tab">
        <v-tab v-for="file in sortedFiles" :key="file.name">{{file.name}}</v-tab>
      </v-tabs>
    </v-toolbar>
